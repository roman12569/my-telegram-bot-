...
if not TOKEN or not MONGO_URL:
    raise ValueError("BOT_TOKEN or MONGO_URL is missing in Environment Variables!")
...
live_check_executor = concurrent.futures.ThreadPoolExecutor(max_workers=2)
cache_executor = concurrent.futures.ThreadPoolExecutor(max_workers=2)
...
def check_live_account(uid):
    try:
        ...
        # CLOUD IP BLOCK FIX
        if res.status_code == 403 or "login" in res.url.lower() or "checkpoint" in res.url.lower():
            if "c_user" not in res.url and clean_uid not in res.url: return False, "Cloud Blocked"
...
def check_ig_username_live(username):
    try:
        ...
        if res.status_code == 403 or "login" in res.url.lower(): return False, "Cloud Blocked"
...
def admin_sub_finance_keyboard():
    try: pc = withdrawals_col.count_documents({"status": "Pending"})
    except: pc = 0
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
...
def _process_callbacks(call):
    ...
    try:
        if code == "verify_join":
            ...
            email_addr = code.replace("check_otp_", "")
            try:
                user, domain = email_addr.split("@")
                res = requests.get(f"https://www.1secmail.com/api/v1/?action=getMessages&login={user}&domain={domain}", timeout=5.0).json()
                if not res:
                    bot.answer_callback_query(call.id, "📭 কোনো নতুন ইমেইল পাওয়া যায়নি! কিছুক্ষণ পর আবার চেষ্টা করুন।", show_alert=True)
                else:
                    msg_id = res[0]['id']
                    msg_detail = requests.get(f"https://www.1secmail.com/api/v1/?action=readMessage&login={user}&domain={domain}&id={msg_id}", timeout=5.0).json()
                    body = msg_detail.get('textBody', 'খালি বার্তা')
                    bot.send_message(chat_id, f"📩 <b>ইমেইল মেসেজ এসেছে:</b>\n\n<b>From:</b> {msg_detail.get('from')}\n<b>Subject:</b> {msg_detail.get('subject')}\n\n<code>{sanitize_html(body[:1000])}</code>")
            except Exception:
                bot.answer_callback_query(call.id, "❌ সার্ভার ব্যস্ত আছে, কিছুক্ষণ পর চেষ্টা করুন!", show_alert=True)
            ...
            bot.edit_message_text("🗑️ কাস্টম পাসওয়ার্ড মুছে ফেলা হয়েছে!", chat_id, call.message.message_id)
            ...
            def notify_rejected_users(stuck_items):
                for s in stuck_items:
                    ...
                    time.sleep(0.05)
                gc.collect() # Clean memory after heavy loop
...
def _process_document(message):
    ...
    worker_name = sanitize_html(message.from_user.first_name if message.from_user else "Worker")
    ...
    if st and st.get('step') == 'AWAITING_BUYER_REPORT' and chat_id == ADMIN_ID:
        ...
        try:
            ...
            bot.send_message(chat_id, f"❌ ফাইল রিড করা যায়নি। কারণ: {str(e)[:100]}")
        finally:
            gc.collect()
        ...
        try:
            ...
            def _chk(c):
                cb_res, cb_msg = check_live_account(c["uid"])
                c["is_live"] = cb_res if c["cat_key"] in ["fb_cookie","fb_2fa"] else True
                c["msg"] = cb_msg
                return c
            ...
            for it in live_check_executor.map(_chk, v_cands):
                if it.get("msg") == "Cloud Blocked": cb_count += 1
                if not it.get("is_live"): continue
                r = get_current_task_rate(it["cat_key"])
                try:
                    submissions_col.insert_one({"chat_id": chat_id, "worker_name": worker_name, "uid": it["uid"], "password": it["password"], "payload": it["payload"], "payload_hash": it["payload_hash"], "track_id": generate_tracking_id(), "category_key": it["cat_key"], "rate": r, "status": "Hold", "date_key": now.strftime("%Y-%m-%d"), "date_str": now.strftime("%Y-%m-%d %H:%M:%S")})
                    sc += 1; te += r
            ...
            msg_final = f"🎉 <b>প্রসেস সম্পন্ন!</b>\n✅ গৃহীত: {sc} | 💰 ৳{te:.2f}"
            if cb_count > 0: msg_final += f"\n⚠️ {cb_count} টি আইডি ক্লাউড আইপি লিমিটের কারণে চেক করা যায়নি।"
            bot.send_message(chat_id, msg_final, reply_markup=submit_tasks_keyboard())
        except Exception as e:
            bot.send_message(chat_id, f"❌ ফাইল প্রসেসিং এরর! কারণ: {str(e)[:100]}")
        finally:
            gc.collect()
...
def _process_main_router(message):
    ...
    if text == "❌ বাতিল করুন": return bot.send_message(chat_id, "❌ বাতিল করা হলো।", reply_markup=main_bottom_keyboard(chat_id))
        ...
        p = get_setting("pass_rule", "None").strip()
        cp = u.get("custom_password", "")
        m = InlineKeyboardMarkup().add(InlineKeyboardButton("✏️ কাস্টম পাসওয়ার্ড সেট করুন", callback_data="user_set_custom_pass"))
        if cp: m.add(InlineKeyboardButton("🗑️ কাস্টম পাসওয়ার্ড মুছুন", callback_data="user_remove_custom_pass"))
        msg = f"⚙️ <b>পাসওয়ার্ড সিকিউরিটি নিয়ম:</b>\n\n• গ্লোবাল রুল: <code>{p}</code>\n• আপনার কাস্টম পাসওয়ার্ড: <code>{cp if cp else 'নাই'}</code>"
        return bot.send_message(chat_id, msg, reply_markup=m)
        ...
        if parse_iso_datetime(lb) and (get_bd_time() - parse_iso_datetime(lb)) < timedelta(hours=24): return bot.send_message(chat_id, "⚠️ ২৪ ঘণ্টায় একবার!")
        ...
        top = list(submissions_col.aggregate([
            {"$match": {"worker_name": {"$ne": None}}},
            {"$group": {"_id": "$worker_name", "count": {"$sum": 1}}},
            {"$sort": {"count": -1}},
            {"$limit": 10}
        ]))
        if not top:
            return bot.send_message(chat_id, "🏆 <b>লিডারবোর্ড খালি!</b> এখনো কোনো টাস্ক সম্পূর্ণ হয়নি।")
        bdg = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
        out = "🏆 <b>LEADERBOARD</b>\n\n"
        for i, item in enumerate(top):
            name = item.get("_id") or "Unknown Worker"
            out += f"{bdg[i]} {name} — {item['count']} টি\n"
        ...
        return bot.send_message(chat_id, f"✉️ <b>Temp Email:</b> <code>{em}</code>\n\nউপরে দেওয়া ইমেইলটি ব্যবহার করুন এবং ইমেইল আসার পর <b>ইনবক্স</b> বাটনে ক্লিক করুন।", reply_markup=m)
    ...
    if sp == 'AWAITING_USER_SET_PASS':
        custom_p = text.strip()
        pr = str(get_setting("pass_rule", "")).strip()
        if pr and pr.lower() != "none" and not validate_strict_password(custom_p, pr):
            return bot.send_message(chat_id, f"⚠️ পাসওয়ার্ড নিয়ম মানা হয়নি! পাসওয়ার্ডের শেষে <b>{sanitize_html(pr)}</b> থাকা আবশ্যক।", reply_markup=cancel_keyboard())
        update_user_field(chat_id, "custom_password", custom_p)
        user_states.pop(chat_id, None)
        return bot.send_message(chat_id, f"✅ আপনার কাস্টম পাসওয়ার্ড সেট করা হয়েছে: <code>{sanitize_html(custom_p)}</code>", reply_markup=main_bottom_keyboard(chat_id))

    elif sp == 'AWAITING_NEW_RATE' and chat_id == ADMIN_ID:
        try:
            r = get_setting("rates", {"fb_cookie": 5.0, "fb_2fa": 6.0, "ig_cookie": 8.0, "ig_2fa": 10.0})
        ...
        def rb():
            try:
                ...
                for i, usr in enumerate(users, 1):
                    try: bot.send_message(usr["_id"], text)
                    except: pass
                    # BROADCAST BATCHING FOR RATE LIMIT
                    if i % 100 == 0:
                        time.sleep(2)
                        try: bot.send_message(ADMIN_ID, f"📢 ব্রডকাস্ট: {i}/{total} সম্পন্ন")
                        except: pass
                try: bot.send_message(ADMIN_ID, f"✅ ব্রডকাস্ট সম্পূর্ণ হয়েছে!")
                except: pass
                gc.collect()
        ...
        def rc():
            try:
                ...
                bot.send_message(chat_id, f"📊 <b>FB CHECK RESULT</b>\nLive: {len(ls)}\n" + "\n".join(ls)[:3500])
            except Exception as e:
                bot.send_message(chat_id, f"❌ চেকিং এ ত্রুটি হয়েছে: {e}")
        ...
        def ri():
            try:
                ...
                bot.send_message(chat_id, f"📊 <b>IG CHECK RESULT</b>\nLive: {len(ls)}\n" + "\n".join(ls)[:3500])
            except Exception as e:
                bot.send_message(chat_id, f"❌ চেকিং এ ত্রুটি হয়েছে: {e}")
        ...
        withdrawals_col.insert_one({"withdraw_id": wid, "chat_id": chat_id, "worker_name": worker_name, "method": st['method'], "account": st['account'], "amount": ra, "status": "Pending", "time": get_bd_time().strftime("%Y-%m-%d %H:%M:%S")})
        ...
        def rbk():
            try:
                ...
                def _chk(i):
                    cb_res, cb_msg = check_live_account(i["uid"])
                    i["il"] = cb_res if i["ck"] in ["fb_cookie","fb_2fa"] else True
                    i["msg"] = cb_msg
                    return i
                    
                cis = list(live_check_executor.map(_chk, pis))
                sc, te, cb_count, now = 0, 0.0, 0, get_bd_time()
                for i in cis:
                    if i.get("msg") == "Cloud Blocked": cb_count += 1
                    if i.get("il"):
                        r = get_current_task_rate(i["ck"])
                        try:
                            submissions_col.insert_one({"chat_id": chat_id, "worker_name": worker_name, "uid": i["uid"], "password": pw, "payload": i["line"], "payload_hash": i["ph"], "track_id": generate_tracking_id(), "category_key": i["ck"], "rate": r, "status": "Hold", "date_key": now.strftime("%Y-%m-%d"), "date_str": now.strftime("%Y-%m-%d %H:%M:%S")})
                            sc += 1; te += r
                        except DuplicateKeyError: pass
                users_col.update_one({"_id": chat_id}, {"$inc": {"hold_balance": te}})
                
                msg_final = f"🎉 <b>বাল্ক সাবমিশন!</b>\n✅ গৃহীত: {sc}\n💰 ৳{te:.2f}"
                if cb_count > 0: msg_final += f"\n⚠️ {cb_count} টি আইডি ক্লাউড আইপি লিমিটের কারণে চেক করা যায়নি।"
                bot.send_message(chat_id, msg_final)
            except Exception as e:
                bot.send_message(chat_id, f"❌ প্রসেসিং এরর: {e}")
            finally:
                gc.collect()
        ...
        if spw and validate_strict_password(spw, pr):
            ...
            try:
                submissions_col.insert_one({"chat_id": chat_id, "worker_name": worker_name, "uid": uid, "password": spw, "payload": text, "payload_hash": generate_payload_hash(text), "track_id": generate_tracking_id(), "category_key": c, "rate": r, "status": "Hold", "date_key": now.strftime("%Y-%m-%d"), "date_str": now.strftime("%Y-%m-%d %H:%M:%S")})
        ...
        r = get_current_task_rate(c)
        try:
            submissions_col.insert_one({"chat_id": chat_id, "worker_name": worker_name, "uid": uid, "password": mp, "payload": pl, "payload_hash": generate_payload_hash(pl), "track_id": generate_tracking_id(), "category_key": c, "rate": r, "status": "Hold", "date_key": now.strftime("%Y-%m-%d"), "date_str": now.strftime("%Y-%m-%d %H:%M:%S")})
            users_col.update_one({"_id": chat_id}, {"$inc": {"hold_balance": r}})
...
@flask_app.route('/')
def flask_home(): 
    return "OEB NEXUS Cyber-AI Production Engine Active (Polling Mode)!"

if __name__ == "__main__":
    print("OEB NEXUS Enterprise Engine Starting...")
    
    def run_web():
        try:
            from waitress import serve
            serve(flask_app, host="0.0.0.0", port=int(os.environ.get("PORT", 10000)), threads=50)
        except ImportError: 
