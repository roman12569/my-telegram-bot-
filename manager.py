...
def send_welcome(message):
    ...
    if not check_force_join(chat_id):
        bot.send_message(chat_id, "🔒 **Channel Verification Required / চ্যানেল জয়েন করুন:**", reply_markup=force_join_keyboard())
        return
...
def main_router(message):
    ...
    if text in ["⚡ ID Submission ⚡", "⚡ আইডি সাবমিশন ⚡"]:
        ...
        user_states.pop(chat_id, None)
        txt = "🛠️ **WORKER HELPER SUITE**\nSelect tool from below:" if lang == 'en' else "🛠️ **ওয়ার্কার হেল্পার টুলস**\nপ্রয়োজনীয় টুলটি বেছে নিন:"
