...
def _process_callbacks(call):
    ...
    if code == "verify_join":
        ...
        for c in cats:
            ...
            if hq > 0 or submissions_col.count_documents({'date_key': dt, 'status': 'Approved', 'category_key': c}) > 0:
                out += f"• {c}: ⏳{hq} / ✅{submissions_col.count_documents({'date_key': dt, 'status': 'Approved', 'category_key': c})}\n"
                th += hq
